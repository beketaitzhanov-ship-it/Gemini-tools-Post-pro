...
# Эта функция УЖЕ поддерживает разные склады (GZ, IW), если они есть в config.json
...
async def save_contract_final_with_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- ИСПРАВЛЕНО: Получаем ссылку на файл ---
...
async def update_status(u, c):
    ...
    if cn:
        ...
        # Заметка: Это должен обрабатывать ДРУГОЙ сценарий в Make, не "Сценарий 2"
        notify_make({
            "event": "status_update",
            "track_number": t,
            "status": st
