...
# Состояния для приемки по договору
...
class GuangzhouBot:
    ...
    async def update_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        track = update.message.text.strip().upper()
        if track.startswith("➕") or track.startswith("📋") or track.startswith("🚚") or track.startswith("🛃") or track.startswith("✅"): return ConversationHandler.END
