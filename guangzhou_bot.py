...
# --- КАТЕГОРИИ (Синхронизировано с Клиентским ботом) ---
CATEGORY_BUTTONS = {
...
def calculate_t1_full(weight, volume, category_key, warehouse_code, agreed_rate_min=0):
    """
    Расчет стоимости.
    Если is_cbm=True, считаем по кубам. Иначе по кг.
    Берет MAX между расчетным тарифом и тем, что согласовал админ (agreed_rate_min).
    """
...
# --- СЦЕНАРИЙ 1: ПРИЕМКА ОЖИДАЕМОГО (ОТ АДМИНА) ---

async def show_expected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    conn = get_db_connection()
    if not conn: return
    cur = conn.cursor()
    # Показываем последние 15 оформленных заявок
    ...
    keyboard = []
    for row in rows:
        # row[0]=CN, row[1]=Name, row[2]=Product
        text = f"{row[1]} | {row[2]}"
    ...
    if conn:
        ...
        if row:
            ...
            await query.edit_message_text(
                f"📥 <b>Приемка: {cn}</b>\n"
                f"🏭 Склад плана: <b>{wh_name}</b>\n"
                f"👤 {row[0]}\n📦 {row[2]}\n\n"
                f"⚖️ <b>Введите ФАКТИЧЕСКИЙ ВЕС (кг):</b>", 
                parse_mode='HTML')
...
async def get_actual_volume(u, c):
    ...
    await u.message.reply_text(
        f"✅ Вес: {d['fact_w']} кг | V: {v:.3f} м³\n"
        f"💰 База: ${cost}\n\n"
        f"🛠 <b>Нужны доп. услуги (упаковка/обрешетка)?</b>\n"
        f"👉 Если да — введите сумму ($)\n👉 Если нет — напишите 0", 
        parse_mode='HTML')
...
async def save_contract_final(u, c):
    ...
    # Генерация трека: КОД_СКЛАДА + ЦИФРЫ (GZ123456)
    prefix = d['wh']
    ...
    notify_make_update({
        "action": "update", "contract_num": d['cn'], "track": track,
        "actual_weight": d['fact_w'], "actual_volume": d['fact_v'], 
        "total_price": total_price, "status": status, "media_link": media_link
    })
...
# --- СЦЕНАРИЙ 2: НОВЫЙ ГРУЗ (С ВЫБОРОМ СКЛАДА) ---
...
async def new_cargo_finish(u, c):
    ...
    # Генерация трека на основе выбранного склада (IW..., FS..., GZ...)
...
async def update_status(u, c):
    ...
    if "ОТПРАВЛЕНО" in mode: 
        st = "В пути (Китай)"
        pr = 40 
    elif "ГРАНИЦЕ" in mode: 
        st = "На границе (Хоргос)"
        pr = 70
    elif "ДОСТАВЛЕНО" in mode: 
        st = "Прибыл в Алматы"
        pr = 100
    else:
        st = "В пути"
    ...
    if conn:
        ...
        for t in tracks:
            cur.execute("""
                UPDATE shipments SET status=%s, route_progress=%s 
