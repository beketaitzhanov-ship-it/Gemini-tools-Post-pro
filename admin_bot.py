...
# 🔥 СБРОС ВЕБХУКА (защита от конфликтов)
def force_delete_webhook(token):
    try:
        url = f"https://api.telegram.org/bot{token}/deleteWebhook?drop_pending_updates=True"
        requests.get(url)
        logger.info("♻️ Вебхук сброшен.")
...
def get_financial_report():
    ...
    try:
        ...
        # Проверка на наличие таблицы расходов (чтобы не падал, если ее нет)
        try:
...
async def show_stats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not data:
        await query.edit_message_text("⚠️ Нет данных или ошибка подключения к базе.")
        return
...
async def get_volume(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if LogisticsCalculator:
            ...
            # Считаем тариф. Город назначения пока ставим "Алматы" по умолчанию
            res = calc.calculate_all(context.user_data['c_weight'], vol, context.user_data['c_cargo'], "Алматы")
            
            context.user_data['calc_rate'] = res['tariff_rate']
            context.user_data['calc_total'] = res['total_usd']
            context.user_data['c_density'] = res['density'] # ✅ ЗАПОМИНАЕМ ПЛОТНОСТЬ
...
async def get_manual_rate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # При ручном вводе считаем плотность сами, чтобы не было 0 в базе
        if context.user_data['c_volume'] > 0:
             context.user_data['c_density'] = round(weight / context.user_data['c_volume'], 2)
        else:
             context.user_data['c_density'] = 0
...
async def generate_contract(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ ИСПРАВЛЕНИЕ: Добавили секунды в номер (%M%S) - теперь они уникальны
    cn = f"CN-{datetime.now().strftime('%m%d%H%M%S')}"
    
    payload = {
        "contract_num": cn,
        "date": datetime.now().strftime("%d.%m.%Y"),
        "client_name": context.user_data.get('c_name'),
        "client_phone": context.user_data.get('c_phone'),
        "city": context.user_data.get('c_city'),
        "cargo_name": context.user_data.get('c_cargo'),
        "weight": context.user_data.get('c_weight'),
        "volume": context.user_data.get('c_volume'),
        "density": context.user_data.get('c_density', 0), # ✅ ИСПРАВЛЕНИЕ: Берем плотность
        "rate": str(context.user_data.get('final_rate')),
        "total_sum": f"{context.user_data.get('final_total')} (Предварительно)",
        "additional_services": "По факту / Upon arrival",
        "clean_total": context.user_data.get('final_total'),
    ...
    try:
        if MAKE_CONTRACT_WEBHOOK:
            ...
            if db_success:
                ...
                await query.message.reply_text(f"⚠️ PDF будет отправлен, но **ОШИБКА БАЗЫ**. Проверь логи Render!")
        else:
            await query.message.reply_text("❌ Ошибка: Нет ссылки на Make в настройках.")
    except Exception as e:
        await query.message.reply_text(f"❌ Ошибка отправки в Make: {e}")
